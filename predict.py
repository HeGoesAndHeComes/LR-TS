...
def predict(bestMoC,total,model, vid_list, args, obs_p, n_class, actions_dict, device):
    # print(f'>>>>>>>>>>>>>>>>>>>>>>>>>>>>> [ model : {args.n_encoder_layer}-{args.n_decoder_layer}-{args.n_head}-{args.n_query}-{args.hidden_dim}-split : ' +args.split+ ' predict start ] <<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
    # print(f'{args.n_encoder_layer}-{args.n_decoder_layer}-{args.n_head}-{args.n_query}-{args.hidden_dim}-split : '+args.split)
    model.eval()
    with torch.no_grad():
        data_path = './datasets'
        ...
        return total,mocs,args.checkpoint
